...
WITH seating_types AS (
   SELECT DISTINCT
       ...
       CASE WHEN UPPER(TRIM(CAST(approved_for_sidewalk_seating AS STRING))) = 'YES' THEN TRUE
            ELSE FALSE
       END AS approved_for_sidewalk,
       
       CASE WHEN UPPER(TRIM(CAST(approved_for_roadway_seating AS STRING))) = 'YES' THEN TRUE
            ELSE FALSE
